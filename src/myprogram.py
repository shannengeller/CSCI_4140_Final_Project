...
if __name__ == '__main__':
    ...
    english_text = "corpus/english/autumn-leaves"
    raw_text_english = open(english_text, "r", encoding= 'utf-8').read()
    raw_text_lowercase_english = raw_text_english.lower()

    # opening french text file
    french_text = "corpus/French/Germaine.txt"
    raw_text_french = open(french_text, "r", encoding= 'utf-8').read()
    ...
    x = np.zeros((len(sentences), seq_length, eng_vocab), dtype=np.bool)
    ...
    # Saves fit so code can be run later without have to do all the epochs again
    ...
    # load pretrained/presaved model to save time
    filename = "english.h5"
    ...
    top_three = np.argpartition(preds, -3)[-3:]
    for each in top_three:
        print(num_to_eng_chars[each])
